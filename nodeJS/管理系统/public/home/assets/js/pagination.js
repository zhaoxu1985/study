function Pagination(id, options) {
  ...
  this.textInfo = {
    首页: options ? options.首页 || '首页' : '首页',
    上一页: options ? options.上一页 || '上一页' : '上一页',
    list: '',
    下一页: options ? options.下一页 || '下一页' : '下一页',
    末页: options ? options.末页 || '末页' : '末页'
  }
...
Pagination.prototype.dongcidaci = function () {
  this.ele.addEventListener('click', e => {
    ...
    if (target.className === '首页' && this.pageInfo.pagenum !== 1) {
      this.pageInfo.pagenum = 1
      this.createPagelist()
      this.isDisable()
    }
    if (target.className === '上一页' && this.pageInfo.pagenum !== 1) {
      this.pageInfo.pagenum--
      this.createPagelist()
      this.isDisable()
    }
    if (target.className === '下一页' && this.pageInfo.pagenum !== this.pageInfo.totalpage) {
      this.pageInfo.pagenum++
      this.createPagelist()
      this.isDisable()
    }
    if (target.className === '末页' && this.pageInfo.pagenum !== this.pageInfo.totalpage) {
